...
class UserGridPresenter extends CoreGridPresenter
{
    ...
    protected function visibleEdit($item)
    {
        if ($this->isUserSystemAdmin()) {
            return $item->role_rank >= $this->getUserLoggedRoleRank() && parent::visibleEdit($item);
        } else {
            return $item->status != 1 && $item->role_rank >= $this->getUserLoggedRoleRank() && parent::visibleEdit($item);
        }
...
//    }
    ...
    public function getUserLoggedRoleRank()
    {
        return Auth::user()->roles()->min('role_rank');
    }

    public function isUserSystemAdmin()
    {
